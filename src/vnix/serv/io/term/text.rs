        ...
        thread!({
            if let Some((s, msg)) = msg.clone().as_pair() {
                if let Some((s, ath)) = as_async!(s, as_str, ath, orig, kern)? {
                    match s.as_str() {
                        // (say <unit>)
                        "say" => return Self::say(false, false, None, None, ath, orig, msg, kern).await,
                        // (say.fmt [<unit> ..])
                        "say.fmt" => return Self::say(false, true, None, None, ath, orig, msg, kern).await,
                        _ => ()
                    }
                }
            }
            ...
            if let Some(_msg) = msg.clone().as_map_find("say") {
                ...
                return Self::say(nl, false, shrt, nice, ath, orig, _msg, kern).await
            }
            ...
            if let Some((lst, mut ath)) = as_map_find_as_async!(msg, "say.fmt", as_list, ath, orig, kern)? {
                ...
                return Self::say(nl, true, shrt, nice, ath, orig, Unit::list_share(lst), kern).await
            }
